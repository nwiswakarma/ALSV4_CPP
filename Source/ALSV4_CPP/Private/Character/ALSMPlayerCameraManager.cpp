...
void AALSMPlayerCameraManager::OnPossess(AALSMBaseCharacter* NewCharacter)
{
	// Set "Controlled Pawn" when Player Controller Possesses new character.
    // (called from Player Controller)
	check(NewCharacter);
	ControlledCharacter = NewCharacter;

	// Update references in the Camera Behavior AnimBP.
	UALSMPlayerCameraBehavior* CastedBehv = Cast<UALSMPlayerCameraBehavior>(CameraBehavior->GetAnimInstance());
	if (CastedBehv)
	{
...
float AALSMPlayerCameraManager::GetCameraBehaviorParam(FName CurveName) const
{
...
void AALSMPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
...
FVector AALSMPlayerCameraManager::CalculateAxisIndependentLag(
...
bool AALSMPlayerCameraManager::CustomCameraBehavior(
    float DeltaTime,
    FVector& Location,
    FRotator& Rotation,
    float& FOV
    )
{
	...
	// Get the 3P Pivot Target (Green Sphere) and
    // interpolate using axis independent lag for maximum control.
	const FVector LagSpd(
        GetCameraBehaviorParam(FName(TEXT("PivotLagSpeed_X"))),
	...
	PivotLocation =
        ...
        + UKismetMathLibrary::GetUpVector(SmoothedPivotTarget.Rotator())
            * GetCameraBehaviorParam(FName(TEXT("PivotOffset_Z")));
	...
	TargetCameraLocation = UKismetMathLibrary::VLerp(
		PivotLocation
        + UKismetMathLibrary::GetForwardVector(TargetCameraRotation)
            * GetCameraBehaviorParam(FName(TEXT("CameraOffset_X")))
        + UKismetMathLibrary::GetRightVector(TargetCameraRotation)
            * GetCameraBehaviorParam(FName(TEXT("CameraOffset_Y")))
	...
	// Step 6: Trace for an object between the camera
    // and character to apply a corrective offset.
    //
	// Trace origins are set within the Character BP via the Camera Interface.
	// Functions like the normal spring arm,
	...
	FTransform FPTargetCameraTransform(
        TargetCameraRotation,
        FPTarget,
        FVector::OneVector
        );

	const FTransform MixedTransform(
